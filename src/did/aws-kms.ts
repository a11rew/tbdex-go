// source: https://github.com/TBD54566975/web5-js/blob/b97dace9af071fa2f9e7464a3d71ad3dfeb56dd0/packages/crypto-aws-kms/src/key-manager.ts#L103
// Extended here to add importKey and exportKey methods

import { CreateAliasCommand, CreateKeyCommand, GetPublicKeyCommand, KeySpec, KMSClient } from '@aws-sdk/client-kms';
import {
...
export class AwsKeyManager implements CryptoApi<AwsKeyManagerGenerateKeyParams> {
	 ...
	 */
	public async digest({ algorithm, data }: AwsKeyManagerDigestParams): Promise<Uint8Array> {
	 ...
	 */
	public async generateKey({ algorithm }: AwsKeyManagerGenerateKeyParams): Promise<KeyIdentifier> {
	...
	public async getPublicKey({ keyUri }: KmsGetPublicKeyParams): Promise<Jwk> {
		...
		console.log('public key', new TextDecoder().decode(response.PublicKey));

		// Convert the public key from SPKI (DER-encoded X.509) to JWK format.
		const publicKey = await convertSpkiToPublicKey({ spki: response.PublicKey });
	...
	/**
	 * Exports a public key identified by the provided key URI from AWS KMS.
	 *
	 * @remarks
	 * This method retrieves the public key from AWS KMS and returns it as a JWK.
	 * Note that AWS KMS does not allow export of private keys for security reasons.
	 *
	 * @example
	 * ```ts
	 * const keyManager = new AwsKeyManager();
	 * const keyUri = await keyManager.generateKey({ algorithm: 'ES256K' });
	 * const publicKey = await keyManager.exportKey({ keyUri });
	 * ```
	 *
	 * @param params - Parameters for exporting the key.
	 * @param params.keyUri - The key URI identifying the key to export.
	 *
	 * @returns A Promise resolving to the JWK representation of the exported public key.
	 */
	public async exportKey({ keyUri }: KmsExportKeyParams): Promise<Jwk> {
		return this.getPublicKey({ keyUri });
	}

	/**
	 * Imports a private key into AWS KMS.
	 *
	 * @remarks
	 * This method imports the provided JWK into AWS KMS, making it available for subsequent
	 * cryptographic operations. It's important to note that AWS KMS doesn't allow direct import
	 * of private key material for asymmetric keys. Instead, this method creates a new KMS key
	 * and associates it with the imported public key.
	 *
	 * @example
	 * ```ts
	 * const keyManager = new AwsKeyManager();
	 ...
	 * @param params.key - The private key to import in JWK format.
	 *
	...
	public async importKey({ key }: KmsImportKeyParams): Promise<KeyIdentifier> {
		...
		// Compute the JWK thumbprint to use as the key ID
		const keyId = await computeJwkThumbprint({ jwk: key });

		// Create a new KMS key
		const createKeyCommand = new CreateKeyCommand({
			KeySpec: supportedAlgorithms['ES256K'].keySpec,
			KeyUsage: 'SIGN_VERIFY',
			Description: 'Imported key',
		});

		const createKeyResponse = await this._kmsClient.send(createKeyCommand, {});
		...
		// Create an alias for the key using the JWK thumbprint
		const createAliasCommand = new CreateAliasCommand({
			AliasName: `alias/urn-jwk-${keyId}`,
			TargetKeyId: kmsKeyId,
		});
		await this._kmsClient.send(createAliasCommand);

		// Construct the key URI
		const keyUri = `${KEY_URI_PREFIX_JWK}${keyId}`;
	 ...
	 */
	public async verify({ key, signature, data }: KmsVerifyParams): Promise<boolean> {
	 ...
	 */
	private getAlgorithm({ algorithm }: { algorithm: SupportedAlgorithm }) {
		// Check if algorithm is supported.
		let AlgorithmImplementation = supportedAlgorithms[algorithm]?.['implementation'];
		if (!AlgorithmImplementation) {
			// throw new Error(`Algorithm not supported: ${algorithm}`);
			// Default to ES256K
			AlgorithmImplementation = supportedAlgorithms.ES256K.implementation;
		}
	 ...
	 */
	private getAlgorithmName({ key, keySpec }: { key?: { alg?: string; crv?: string }; keySpec?: KeySpec }): SupportedAlgorithm {
...
export async function convertSpkiToPublicKey({ spki }: { spki: Uint8Array }): Promise<Jwk> {
	// Use the Web Crypto API to import the public key from the SPKI byte array
	// Note: We're using 'P-256' here as it's widely supported. 'secp256k1' isn't directly supported in Web Crypto API.
	...
	// Modify the JWK to represent secp256k1 curve
	const secp256k1Jwk = {
		...jwk,
		crv: 'secp256k1',
		kty: 'EC',
	...
	// Cast the exported key to Jwk type
	return secp256k1Jwk as Jwk;
}
